...
func (cmd *GenerateModelsCommand) init() error {
	...
	var e error
	cmd.templateHeader, e = template.New("modelsheaderText").Funcs(funcs).Parse(modelsheaderText)
	if nil != e {
		return e
	}

	cmd.templateModel, e = template.New("modelText").Funcs(funcs).Parse(modelText)
	if nil != e {
		return e
	}

	cmd.templateModel.New("toNullValue").Parse(sqlNullValueText)
	if nil != e {
		return e
	}
	return nil
...
var modelsheaderText = `// file is generated by gengen
package {{.Namespace}}
...
var modelText = `type {{.table.ClassName}} struct { {{range $x := .columns }}{{if eq "Id" $x.GoName}}
...
var sqlNullValueText = `{{if eq .DbType "bool"}}if {{toNullName .DbName}}.Valid { 
